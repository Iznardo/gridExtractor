...
def _process_one_scrim_game(
    ...
    *,
    ...
    # series-started-game y rescata side-swaps internamente (Patron D resuelto
    # en la libreria), asi que ya no hay rescate manual desde draft_history. Si
    # aun asi no hay draft (Patron A/B/C), se salta la partida.
    draft_data = draft.get_draft()
    if not draft_data["draft_found"]:
        log.warning("Scrim %s game %d: draft vacio (sin eventos de draft), "
